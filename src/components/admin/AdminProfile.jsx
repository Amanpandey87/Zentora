...
const AdminProfile = () => {
  ...
  const [profileImage, setProfileImage] = useState(info?.avatar || '')
  ...
  const handleSavePhoto = () => {
    if (!profileImage) {
      Swal.fire({ title: 'Profile photo', text: 'Please select an image first.', icon: 'warning' })
      return
    }

    const storedUser = JSON.parse(localStorage.getItem('info') || '{}')
    const updatedUser = { ...storedUser, avatar: profileImage }
    localStorage.setItem('info', JSON.stringify(updatedUser))

    Swal.fire({ title: 'Profile photo', text: 'Profile photo updated successfully.', icon: 'success' })
  }
  ...
  const onSubmit = async (data) => {
    ...
    const res = await axios.put('/admin-profile-update', finalData)
                  ...
                  <div className="row align-items-center g-3">
                    <div className="col-12 col-sm-3">
                      <div className="profile-avatar-wrap">
                        {profileImage ? <img src={profileImage} alt="Profile avatar" /> : 'P'}
                      </div>
                    </div>
                    <div className="col-12 col-sm-9">
                      <p className="text-secondary mb-3">Upload a clear profile photo. JPG, PNG, or other standard image formats are supported.</p>
                      <div className="row g-2">
                        ...
                        <div className="col-auto">
                          <button type="button" className="btn btn-orange btn-sm" onClick={handleSavePhoto}>Save Photo</button>
                        </div>
                      </div>
                    </div>
                  </div>
